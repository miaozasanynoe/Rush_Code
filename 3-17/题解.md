...
这个算法的时间复杂度是O(n),然后超时了，题目中给出的数据范围是0- 2^31,数据太大，遍历一遍肯定会超时，所以应该是要找规律，那么我们来推一下规律，当0-9时每个数占一位。10-99占两位，100-999每个数占三位，以此类推第n个数占ceil(lg10 n)倍，这样我们就可以先找到占位数，再得到答案。
```
class Solution {
public:
    int findNthDigit(int n) {
